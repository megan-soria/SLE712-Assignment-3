...
A file can de downloaded from an external URL using the function download.file(). The argument *destfile* can be used to specify the name and the destination folder to be used. 
...
### Unzip a .gz file 
The function gunzip() from the R.utils library can compress or decrompress files with *.gzip* and *.bzip2* formats. The argument *overwrite* when set to FALSE does not remove the original compressed file after decompressing.
...
To make a pairwise alignment, we must first convert the target sequence (seq11) which is a vector of characters into a string using the c2s() function. The string of characters will then be converted into a DNAString object by using the DNAString()function. Both of the studied sequences must be converted as shown in the code chunck below. 
...
Now that the sequence are in the appropriate DNAString class, we can now use the pairwiseAlignment() function from the Biostrings library. The subject (in this case the mutated sequence) must be the second input of the function preceeded by the pattern (in this case the original sequence) which can be a set of lists.
...
The resulting alignment can now be used to determine the percent sequence identification using the pid() funtion. 

```{r, echo=TRUE}
pid(aln)
```

Or determine the number of mismateches using the nmismatch() function.
